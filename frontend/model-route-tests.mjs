...
const inferenceFile=process.env.BLUECHO_INFERENCE_FILE||'FLS_Debris/propeller.jpg';
const expectedClass=process.env.BLUECHO_EXPECTED_CLASS||'propeller';
...
try{
 ...
 for(const file of files){await page.getByLabel('Sonar source',{exact:true}).setInputFiles(path.join(root,'FLS_Debris',file));await page.locator('.source-card').waitFor();assert.equal(await page.getByLabel('Model route').inputValue(),'fls11-debris','Automatic recognition: '+file);await page.getByLabel('Confirm source modality').selectOption('FLS_ARIS');await page.waitForFunction(()=>document.querySelector('#model-route-status')?.textContent.includes('FLS debris specialist is ready'));assert.equal(await page.getByLabel('Model route').inputValue(),'fls11-debris');assert.ok(await page.getByRole('button',{name:'Start inspection',exact:true}).isEnabled())}
 ...
 await page.getByLabel('Sonar source',{exact:true}).setInputFiles(path.join(root,inferenceFile));await page.locator('.source-card').waitFor();await page.getByRole('button',{name:'Start inspection',exact:true}).click();await page.locator('.candidates h2').waitFor({timeout:180000});assert.ok(await page.locator('svg.sonar [data-candidate]').count()>0);await page.locator('svg.sonar').screenshot({path:path.join(out,'live-boxes.png')});
 await page.getByRole('button',{name:'View inspection report',exact:true}).first().click();const pending=page.waitForEvent('download');await page.getByRole('button',{name:'JSON',exact:true}).click();await(await pending).saveAs(path.join(out,'live.json'));const r=JSON.parse(await fs.readFile(path.join(out,'live.json'),'utf8'));assert.ok(r.detections.some(d=>d.class_name===expectedClass));assert.ok(r.detections.every(d=>d.model_id==='fls11-debris'));assert.ok(!r.demo);assert.deepEqual(errors,[]);
 await fs.writeFile(path.join(out,'receipt.json'),JSON.stringify({status:'PASS',files,checks:['All FLS PNG/JPG model choices survive reselecting FLS','Switching back to FLS restores its detector','Mismatched sensor remains blocked','Fresh selected-image inference produces expected boxes'],errors},null,2));console.log('PASS',files.length,'FLS uploads, sensor reselection, and real inference:',expectedClass);
