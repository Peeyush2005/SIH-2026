...
try{
 ...
 assert.equal(await page.locator('.scene-card').count(),10);assert.equal(await page.getByRole('button',{name:/Open Gulf survey/}).count(),0);
 ...
 for(const width of [1536,1024,768,390]){
  ...
  const reviews=await page.locator('.review-actions button').evaluateAll(es=>es.map(e=>e.getBoundingClientRect().height));assert.equal(reviews.length,4);assert.ok(reviews.every(h=>h===44));
  await page.getByRole('button',{name:'View inspection report',exact:true}).first().click();
  ...
  const bounds=await page.locator('.export-buttons').boundingBox();assert.ok(Math.max(...boxes.map(b=>b.right))<=bounds.x+bounds.width+1);
  ...
  await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.join(out,'page-'+width+'.png'),fullPage:true});await page.getByRole('button',{name:'Return to inspection',exact:true}).click();
 }
 await page.getByRole('button',{name:'View inspection report',exact:true}).first().click();
