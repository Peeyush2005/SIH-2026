...
export const modelName=(id:string)=>({'sss-pipeline-v3':'Pipeline detector','sss-wreck-experimental':'Experimental SSS detector','uatd-fls':'UATD object detector','fls-debris-development':'FLS debris detector'}[id]||id);
export const modalityName=(id:string)=>({'SSS':'Side-scan sonar','SSS_LF':'Side-scan Â· low frequency','FLS_UATD':'Forward-looking Â· UATD','FLS_ARIS':'Forward-looking Â· ARIS'}[id]||id);
...
export const coordinate=(value:number,axis:'lat'|'lon')=>`${Math.abs(value).toFixed(6)}Â° ${axis==='lat'?(value<0?'S':'N'):(value<0?'W':'E')}`;

export function Coverage({browser=false}:{browser?:boolean}){return <div className="coverage-strip" aria-label="Side-scan model coverage"><span className="coverage-title">SSS coverage</span><span><i className="dot"/>Pipelines <small>Development</small></span><span><i className="dot amber"/>Shipwrecks <small>{browser?'Local route only':'Experimental'}</small></span><span><i className="dot gray"/>Cylinders <small>Unvalidated</small></span><span><i className="dot gray"/>Entangled nets <small>Unavailable</small></span></div>}
