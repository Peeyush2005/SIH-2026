...
try{
 ...
 await page.getByRole('button',{name:'Home',exact:true}).click();await page.getByRole('button',{name:'Try demo',exact:true}).click();await page.locator('.demo-guide').waitFor();await page.getByRole('button',{name:'Reports',exact:true}).click();await page.getByRole('button',{name:'Clear demo history',exact:true}).click();await page.getByLabel('Filter inspections').selectOption('uploads');await page.waitForFunction(()=>document.querySelectorAll('.inspection-row').length===2);pass('Clearing demo history preserves both real inspections');
